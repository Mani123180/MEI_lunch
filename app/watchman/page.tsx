...
export default function SimpleWatchman() {
    ...
    const handleAction = (type: 'exit' | 'entry') => {
        ...
        GlobalStore.updatePass(currentPass.id, {
            ...
            status: type === 'entry' ? "used" : "approved",
        });
                ...
                {activeScreen === "details" && currentPass && (
                    <div className="animate-in slide-in-from-bottom-8 duration-500">
                        <div className="bg-white rounded-[3rem] p-10 space-y-10 shadow-2xl overflow-hidden relative border border-gray-50">
                            ...
                            <div className="flex items-center gap-6 border-b pb-10 relative">
                                ...
                                <div className="w-16 h-16 bg-[#1e3a8a] text-white rounded-2xl flex items-center justify-center font-black text-3xl shadow-lg overflow-hidden border-2 border-white/20">
                                ...
                                <div>
                                    <h3 className="text-2xl font-black text-gray-900 mb-1 leading-tight">{studentForPass?.name || "Student Name Missing"}</h3>
                            ...
                            <div className="space-y-4 pt-6">
                                {!currentPass.scannedOutAt ? (
                                    <button onClick={() => handleAction('exit')} className="w-full h-20 bg-orange-600 text-white rounded-[2rem] font-black text-xl shadow-xl shadow-orange-100 active:scale-[0.98] transition-all uppercase tracking-widest">LOG EXIT</button>
                                ) : (
                                    <button onClick={() => handleAction('entry')} className="w-full h-20 bg-[#1e3a8a] text-white rounded-[2rem] font-black text-xl shadow-xl shadow-blue-100 active:scale-[0.98] transition-all uppercase tracking-widest transition-all">LOG ENTRY</button>
                                )}
